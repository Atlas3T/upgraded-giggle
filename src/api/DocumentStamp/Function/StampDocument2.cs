...
namespace DocumentStamp.Function
{
    ...
    public class StampDocument2
    {
        ...
        public StampDocument2(IRpcClient rpcClient, PeerSettings peerSettings, FfiWrapper cryptoContext, IPrivateKey privateKey, PeerId recipientPeer)
        {
            ...
            Microsoft.Extensions.Logging.ILogger log)
        {
            ...
            try
            {
                ...
                var transaction =
                    StampTransactionHelper.GenerateStampTransaction(_privateKey, receiverPublicKey,
                        userProofJson.ToUtf8Bytes(), 1, 1).Transaction;
                var protocolMessage =
                ...
                var signatureBytes = _cryptoContext.Sign(_privateKey, protocolMessage.ToByteArray(),
                ...
                var b = Convert.ToBase64String(protocolMessage.ToByteArray());
            ...
            {
                //await _rpcClient.Channel.EventLoop.ShutdownGracefullyAsync();
                await _rpcClient.Channel.DisconnectAsync();
                await _rpcClient.Channel.CloseAsync();
                await _rpcClient.Channel.DeregisterAsync();
                _rpcClient.Dispose();
                GC.Collect();
            }
        }
